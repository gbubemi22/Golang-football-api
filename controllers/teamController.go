...
func GetTeam() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		defer cancel()
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occured while fetching the team "})
		}
...
func UpdateTeam() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if team.League_id != nil {
			...
			defer cancel()
			if err != nil {
				msg := fmt.Sprintf("message:League was not found")
				c.JSON(http.StatusInternalServerError, gin.H{"error": msg})
				return
			}
		...
		if err != nil {
			msg := fmt.Sprint("team  update failed")
			c.JSON(http.StatusInternalServerError, gin.H{"error": msg})
			return
		}
		c.JSON(http.StatusOK, result)
